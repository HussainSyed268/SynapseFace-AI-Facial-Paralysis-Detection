...
def train_and_save_models(data_paths, labels):
    ...
    # Train the SVM model
    svm_model = SVC(kernel='rbf', C=50000, gamma=0.001 , probability=True )
    svm_model.fit(X_train, y_train)
    models.append(('svm', svm_model))

    # Train a Random Forest model
    random_forest = RandomForestClassifier(n_estimators=800, max_depth=3, random_state=42)
    random_forest.fit(X_train, y_train)
    models.append(('random_forest', random_forest))

    # Train a Gradient Boosting model
    gradient_boosting = GradientBoostingClassifier(n_estimators=800, learning_rate=0.3, max_depth=3, random_state=42)
...
for i in range(1, 1139):
    if ((i >= 345) and (i <= 444)):
        ...
        data_paths.append(f"images2/{i}.jpg")
